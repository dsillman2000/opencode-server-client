# http-client-sync Specification

## Purpose
Provide a synchronous HTTP client for making requests to the OpenCode server with automatic exponential backoff retry logic on transient failures.

## Requirements

### Requirement: Synchronous HTTP Client Creation
The system SHALL provide a `SyncHttpClient` class for making synchronous HTTP requests to the OpenCode server.
