# http-client-async Specification

## Purpose
Provide an asynchronous HTTP client for making async/await requests to the OpenCode server with automatic exponential backoff retry logic on transient failures.

## Requirements

### Requirement: Asynchronous HTTP Client Creation
The system SHALL provide an `AsyncHttpClient` class for making asynchronous HTTP requests to the OpenCode server.
