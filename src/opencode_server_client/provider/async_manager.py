"""Async provider manager for OpenCode server."""
...
class AsyncProviderManager:
    """Manage providers and models from the OpenCode server asynchronously.

    Provides user-friendly APIs for querying available providers, their models,
    and filtering by capabilities.

    Example:
        >>> manager = AsyncProviderManager(http_client)
        >>> providers = await manager.list_providers()
        >>> deepseek = await manager.get_provider("deepseek")
    ...
    def __init__(self, http_client: AsyncHttpClient):
        """Initialize the AsyncProviderManager.
    ...
    async def _fetch_providers(self, use_cache: bool = True) -> ProviderList:
        ...
        response = await self.http_client.get("/provider")
    ...
    async def list_providers(self, connected_only: bool = False) -> list[Provider]:
        """Get all providers.
    ...
    async def get_model(self, provider_id: str, model_id: str) -> Optional[Provider]:
    ...
    async def list_text_capable_models(
    ...
    async def is_provider_connected(self, provider_id: str) -> bool:
    ...
    async def refresh_cache(self) -> None:
        ...
        self._cache = None
        await self._fetch_providers(use_cache=False)
