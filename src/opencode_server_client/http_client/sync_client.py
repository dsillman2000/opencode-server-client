"""Synchronous HTTP client with retry logic for OpenCode server communication."""
...
class SyncHttpClient:
    """Synchronous HTTP client with exponential backoff retry logic.

    This client automatically retries transient failures (502, 503, transport errors)
    with exponential backoff. It supports context manager protocol for automatic
    resource cleanup.
    ...
    def get(
        ...
        Returns:
            HTTP response object
        """
        return self._request_with_retry("GET", url, directory=directory, **kwargs)
    ...
    def post(
        ...
        Returns:
            HTTP response object
        """
        return self._request_with_retry("POST", url, directory=directory, **kwargs)
